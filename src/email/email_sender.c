...
static int send_watchdog_email(const char *subject, const char *body_prefix, float cpu_temp)
{
    ...
    snprintf(email_body, sizeof(email_body),
        ...
        "========================================\n\n"
        "This is an automated alert from the Embedded Security System.\n",
        body_prefix, timestamp, cpu_temp);
    ...
    if (res == CURLE_OK) {
        printf("[EMAIL] Watchdog alert sent: %s\n", subject);
    } else {
                     ...
                     const unsigned char *jpeg_buf, size_t jpeg_len)
{
    ...
    int result = send_email_with_subject(
...
{
    // Watchdog debounce (30 seconds)
    time_t now = time(NULL);
    if (now - last_watchdog_email_time < 30) {
        return 0;
    }
    
    int result = send_watchdog_email(
        "WATCHDOG ALERT: Camera TAMPERING Detected!",
        "WATCHDOG: Camera feed has been interrupted or tampered with!\n"
...
int email_send_alert_thermal(float cpu_temp)
{
    ...
    // Debounce: at most 1 thermal email per 60 seconds
    if (now - last_thermal_email_time < 60) {
        return 0;
    }
    ...
    snprintf(full_message, sizeof(full_message), 
        "Subject: THERMAL ALERT: CPU Throttling Started\r\n\r\n"
        ...
        "Threshold: %d C\n"
        "========================================\n\n"
        "The CPU temperature has exceeded the threshold.\n"
        "The system has automatically reduced FPS and resolution to prevent overheating.\n\n"
        "This is an automated alert from the Embedded Security System.\n",
        timestamp, cpu_temp, g_temp_throttle_c);
    ...
    if (res == CURLE_OK) {
        printf("[EMAIL] Thermal throttle alert sent\n");
        last_thermal_email_time = now;
    } else {
        printf("[EMAIL] Failed to send thermal alert: %s\n", curl_easy_strerror(res));
    }
