...
static std::vector<cv::Rect> detectFaces(const cv::Mat &img320)
{
    ...
    // Prepare input names - store them as strings that will persist
    std::vector<std::string> input_names_str;
    std::vector<const char*> input_names;
    Ort::AllocatorWithDefaultOptions allocator;
    
    // Get input names properly
    for (size_t i = 0; i < blaze_session->GetInputCount(); i++) {
    ...
    try {
        ...
        // 896 is the number of detections, 16 is the box coordinates
